...
> **PLAN 이다. 이 문서를 구현된 것으로 읽지 않는다.**
>
> 이 라운드(2026-09-23)는 **문서만** 바꿨다 — 제품 코드 · 렌더러 ·
> Studio UI · validator · 순수 writer · sandbox 프로토콜 ·
> `APP_BUILD_VERSION` 전부 무변경이고 배포도 하지 않았다. 지금 코드가
> 강제하는 것은 [계약 문서](../contracts/IMORY_HOME_CANVAS_CONTRACT.md)
> 이고, 이 문서가 말하는 `canvas.groups` 는 **저장소 어디에도 없다.**
>
