...
> **이 설계는 전부 구현됐다** — `GROUP-1A`(2026-09-23) ·
> `GROUP-1B`(2026-09-24) · `GROUP-1C`(2026-09-24).
>
...
> 전체 이동) · **§40**(그룹 전체 크기 조절 · 회전). 이 문서의 모든
> 절은 이제 그 구현의 **근거 기록**으로 읽는다 — 새 작업의 근거는
> 계약 문서다.
>
> `1C` 가 뒤집은 것은 **하나**다. §4-7 은 "프레임이 멤버별 중심을
> 보고한다"고 했는데, 실제로는 **기준점에서의 차이 벡터와 잰 세로
> 길이**를 보고한다 — 프레임은 저장 좌표계의 원점을 모르므로 중심의
> 절대값을 보내도 부모가 그것을 저장 좌표로 읽을 수 없다(계약 §40-8).
> §4-6 의 `A_m = anchor × r(m)` 도 같은 이유로 **되짚은 기준점**
> (`space.x × unitScale − v`)으로 바뀌었다.
>
...
> **✅ 완료(2026-09-24).** 실제로 강제되는 계약은
> [계약 문서 §40](../contracts/IMORY_HOME_CANVAS_CONTRACT.md) 이다.
> 이 목록에서 **두 줄이 바뀌었다.**
>
> - 메시지 이름은 `v2-group-resize` · `v2-group-rotate` 둘이 아니라
>   **`canvas-group-transform` 하나**다(`kind` 가 종류를 가른다) —
>   자리 관문 · 열린 칸 · 답 번호가 한 벌이면 되기 때문이다.
>   sandbox 봉투는 `IMORY_CANVAS_GROUP_TRANSFORM` 이고, `canvas-group`
>   에는 칸 셋이 늘었다(`scaleMin` · `scaleMax` · `canRotate`).
> - **Moveable 의 group event 는 실측했고 쓰지 않았다.**
>   `resizeGroup` · `rotateGroup` 은 0.53.0 에 있지만, 그 제스처가
>   재는 상자가 우리가 끄는 동안 고치는 자식 DOM 과 물려 돌고
>   손잡이 hit area 가 손가락에 작다. `1B` 와 같이 **입력도 표시도
>   편집 runtime 이 갖는다**(계약 §40-2) — 손잡이 다섯을 control
>   box 안에 직접 만든다.
>
> **더 들어간 것**: 공통 배율 범위(멤버 전부의 교집합 — §40-7) ·
> 끄는 동안의 임시 배율 · 각도 두 칸(§40-10) · 숨은 `"auto"` 멤버가
> 있으면 회전을 막는 규칙(§40-6).

- `canvas-group-transform`(균등 배율 · 모서리 넷 / 피벗 하나 · 멤버별
  차이 벡터) — §4-6 · §4-7
- ~~Moveable 이 실제로 주는 group event 를 실측해서 쓴다~~ → 실측한
  결과 **쓰지 않는다**(위 ★)
- `height:"auto"` 는 크기에서 가로만 받고 회전에서는 **잰 높이**가
  필요하다(§4-6 · §4-7 · 계약 §40-6)
...
>
> **✅ `GROUP-1C`(2026-09-24)가 그 자리를 마저 채웠다.**
> 같은 파일에 `[group-resize]` · `[group-rotate]` · `[sequence]` 세
> 절이 늘었고(같은 포트 9010 · 9011 — 279 검사),
> `skin/sandbox/skin-sandbox-unit-test.mjs` 의 `[canvas-group]` 절에
> 새 봉투(`IMORY_CANVAS_GROUP_TRANSFORM`)와 늘어난 칸 셋이 들어갔다.
> `"groups"` 가 봉투에 없다는 그 단언은 **세 번째 메시지가 생긴
> 지금도 그대로 참**이다.
...
- **`height:"auto"` 멤버가 있는 그룹은 크기 조절 뒤 화면 상자가 배율과
  ...
  → `1C` 에서 그대로 구현됐다(계약 §40-4). 덧붙여 **화면에 없는
  `"auto"` 멤버가 있으면 그 그룹은 회전하지 않는다**(§40-6) — 중심에
  필요한 세로 길이를 아무도 모른다.
