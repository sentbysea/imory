...
단, 공개 페이지(HOME/CATEGORY/POST/BANNER)의 **표시 공간·화면 전환·소유자 관리 동선·Preview 일치**에 관한 상세 규칙은 이 문서가 아니라 [SKIN_SURFACE_AND_TRANSITION_CONTRACT.md](../contracts/SKIN_SURFACE_AND_TRANSITION_CONTRACT.md)가 관리한다. 이 문서는 제품 방향과 단계 계획을 담당한다.
