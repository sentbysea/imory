...
> 무변경**이다) ·
> `HOME-CANVAS-GROUP-1C`(2026-09-24, **그룹 전체 크기 조절 · 회전** — §40.
> 새 sandbox 메시지 하나(`IMORY_CANVAS_GROUP_TRANSFORM`)와 `CANVAS_GROUP`
> 의 칸 셋 · 끄는 동안만 쓰는 화면용 두 칸(배율 · 각도)이 늘었고,
> **저장 구조 · 렌더 payload · v1 계약은 무변경**이다 — 그룹은 여전히
> 숫자를 저장하지 않는다).
> 로드맵: [IMORY_HOME_CANVAS_ROADMAP.md](../plans/IMORY_HOME_CANVAS_ROADMAP.md) — **PLAN**.
> 그룹 설계: [IMORY_HOME_CANVAS_GROUP_DESIGN.md](../plans/IMORY_HOME_CANVAS_GROUP_DESIGN.md) — **1A~1C 구현 완료**.
...
- 스냅 · 가이드 · 키보드 조작 · preset · 스티커 업로드 · widget ·
  다중 일괄 편집 — **하나도 없다**(§11 · §19-12 · §22-7).
  > **✅ 그룹 조작은 생겼다** — `HOME-CANVAS-GROUP-1A`~`1C`(§38 · §39 ·
  > §40: 영구 그룹의 저장 · 폴더 · 전체 이동 · 전체 크기 조절 · 전체
  > 회전). 다만 **Layers 의 폴더로 고른 영구 그룹**에만 있고, lasso 로
  > 여럿을 고른 상태에는 여전히 없다(§16-9).
...
| ~~**그룹 조작**~~ → **`GROUP-1A`~`1C` 가 끝냈다**(§38 · §39 · §40). 다만 **Layers 의 폴더로 고른 영구 그룹**에만 있다 — lasso 로 여럿을 고른 상태는 지금도 틀만 남고 손잡이가 전부 꺼진다(§17-1 · §18-1 · §19-1) | 남은 것은 **변 중앙 손잡이 · 뒤집기 · 중첩 그룹 · 그룹 단위 순서 이동**이고, 아직 어느 라운드에도 올라가 있지 않다(§40-13) |
...
★ **이동은 `GROUP-1B`(§39), 크기 조절 · 회전은 `GROUP-1C`(§40) 에서
열렸다.** 아래 목록의 나머지는 그대로다.

**그룹 전체의 이동(→ §39) · 크기 조절 · 회전(→ §40)** · **그룹 단위
...
- ~~**그룹 크기 조절 · 회전이 없다**(`GROUP-1C`).~~ → **§40 에서 열렸다.**
  Canvas 패널의 안내도 셋을 함께 말한다.
...
## 40. 그룹 전체 크기 조절 · 회전 (`HOME-CANVAS-GROUP-1C`)

Layers 의 폴더를 고른 뒤 Preview 의 **모서리 손잡이 넷**으로 그룹 전체의
크기를 비율을 지킨 채 바꾸고, **위쪽 회전 손잡이**로 그룹 전체를 돌린다.
이것이 `HOME-CANVAS-GROUP-1A`~`1C` 의 마지막 구현이다.

설계: [docs/plans/IMORY_HOME_CANVAS_GROUP_DESIGN.md](../plans/IMORY_HOME_CANVAS_GROUP_DESIGN.md)
§4-6 · §4-7

### 40-1. 무엇이 열렸고 무엇이 아직인가

| | 상태 |
| --- | --- |
| 그룹 전체 이동 | 열렸다(§39) |
| 그룹 전체 **크기 조절**(모서리 넷 · 균등 배율) | **열렸다** |
| 그룹 전체 **회전**(위쪽 손잡이 하나) | **열렸다** |
| 변 중앙 손잡이 · 가로만 · 세로만 늘이기 | **없다**(§40-4 의 ★) |
| 뒤집기(음수 배율) | **없다** |
| 중첩 그룹 · 그룹 단위 순서 이동 | 없다 |
| 그룹 자체의 좌표 · 배율 · 각도 저장 | **없다**(§38-1 그대로 — 그룹은 숫자를 저장하지 않는다) |
| `main_visual` 프레임 자체를 그룹에 넣기 | 없다(블록은 좌표가 없다 — §38-2) |

### 40-2. 손잡이 — 왜 Moveable 의 것이 아닌가

`1B` 가 그룹 drag 에서 내린 결론(§39-2)이 여기서도 그대로다. **입력도
표시도 편집 runtime 이 갖고**(`skin/skin-home-canvas-editor-runtime.js`),
Moveable 은 지금까지처럼 외곽선만 그린다(`renderDirections: []` ·
`rotationPosition: "none"`).

0.53.0 에 `resizeGroup` · `rotateGroup` 이벤트가 **있다**(번들 실측).
쓰지 않은 이유는 셋이다.

1. 그 제스처가 재는 상자는 자식 moveable 들의 `pos1~pos4` 합집합인데,
   우리는 끄는 동안 자식 DOM 을 직접 고친다(임시 배율 · 임시 이동 —
   §40-10). 따라가기 루프가 매 프레임 `updateRect()` 를 부르므로 두
   계산이 같은 값을 물고 돈다.
2. 손잡이를 잡을 수 있는 자리가 **보이는 크기 그대로**(14px)라 손가락
   으로 잡기 어렵다(§40-11).
3. 그룹 제스처를 켜려면 `renderDirections` 를 그룹에도 줘야 하는데, 그
   순간 **단일 선택과 그룹이 같은 prop 을 나눠 쓰게 된다**. 지금은 그
   prop 이 "단일이면 여덟, 그룹이면 없음" 한 줄이다.

그래서 control box 안에 **우리가 손잡이 다섯을 만들어 넣는다**(이동
손잡이와 같은 방법 — `ensureGroupHandles()` · `syncGroupHandles()`).

| | 자리 | 크기 |
| --- | --- | --- |
| 모서리 넷(`nw`·`ne`·`sw`·`se`) | 멤버들의 **실제 상자 합집합**의 네 꼭짓점 | 20px(보이는 크기 = 잡는 크기) |
| 회전 하나 | 그 합집합의 **위쪽 변 중앙에서 36px 위** | 20px · 원형 |
| 이동 손잡이(§30-2) | 합집합의 왼쪽 위 바깥(지금까지 그대로) | 18px |

회전 손잡이와 위쪽 모서리 손잡이는 겹치지 않고(실측 16px 틈), 이동
손잡이와 `nw` 도 겹치지 않는다(실측 1px 틈).

- **잠긴 멤버가 있는 그룹에는 손잡이가 하나도 그려지지 않는다**(§40-6).
- **크기 조절이 불가능한 그룹**(공통 배율 범위가 없다 — §40-7)에는 모서리
  넷이, **회전이 불가능한 그룹**(§40-6)에는 회전 하나가 그려지지 않는다.
- 자식 행을 고르면 지금까지의 **단일 요소 손잡이 여덟 + 회전 하나**로
  돌아온다(§18 · §19 그대로).

### 40-3. geometry 의 기준 — 편집 여유는 geometry 가 아니다

§21-4 · §29-2 가 정한 그대로다. 그룹에서는 애초에 섞일 길이 없다 —
`applyEditChromePadding()` 이 여유(글자 5px)를 **단일 선택에만** 준다
(`elements.length === 1`). 그래서 그룹의 외곽선 · 손잡이 · 조작 기준이
전부 **멤버들의 실제 상자 합집합** 하나다.

| 무엇 | 어디에 쓰나 |
| --- | --- |
| 멤버의 실제 상자(`getBoundingClientRect`) | 합집합 · 고정점 · 피벗 · 손잡이 자리 |
| 그 합집합 | 화면의 배율 · 각도를 재는 자 |
| Moveable 의 선 · 손잡이 · 부모 Studio 의 보조 상자 | **아무 계산에도 쓰지 않는다** |
| 저장 상자(Canvas 좌표) | 부모가 확정하는 값 — §40-4 · §40-5 |

회전한 멤버의 상자는 이미 **회전을 포함한** 바깥 상자이므로, 합집합이
곧 "모든 멤버의 회전된 모서리를 품는 bounds" 다. 회전 전 상자로 세지
않는다.

### 40-4. 크기 조절 — 균등 배율 하나

```text
  A = 잡은 모서리의 **반대편** 꼭짓점(고정점)
  s = 공통 균등 배율

  x' = A + s(x − A)        w' = w × s
  y' = A + s(y − A)        h' = h × s      ("auto" 는 "auto" 그대로)
  rotation 그대로
```

- **어느 모서리를 잡아도 종횡비가 고정**이다. Shift 를 누르지 않아도
  그렇고, 한 축으로만 끌어도 그렇다. 배율은 잡은 모서리와 고정점을 잇는
  **대각선 위로 포인터를 투영**해 만든다 — 대각선에서 벗어난 손떨림이
  배율을 튀게 하지 않는다.
- **음수 배율과 뒤집기가 없다.** 배율은 언제나 양수이고 §40-7 의 범위
  안이다.
- ★ **비균등(sx ≠ sy)을 쓰지 않는 이유**는 회전이다. 회전한 자식을
  비균등으로 줄이면 전단(shear)이 생기고, 계약에 `skew` 칸이 없으므로
  표현할 수 없는 상태가 된다. 이것은 단일 요소의 손잡이 여덟(§18-2)과
  **일부러 다른 규칙**이다.

**`pin` 멤버도 같은 한 줄이다.** 그 요소의 `x` 는 상자의 왼쪽 위가 아니라
`origin` 이 놓일 자리이지만(§24-4), 균등 배율은 아핀 변환이라 상자 위의
어느 점이든 같은 식을 따른다 — `origin` 항이 양변에서 지워진다. 저장으로
옮기는 것은 지금까지의 그 번역 하나다(`planStudioCanvasV2Transform`
`"v2-resize"` → `writeSkinHomeCanvasV2NodePinBox`).

#### 글자는 어떻게 되는가 — **상자만 바뀐다**

★ **크기 조절은 `fontSize` 를 바꾸지 않는다.** 단일 요소 리사이즈(§18)와
**같은 계약**이다 — 바뀌는 것은 저장된 상자 네 칸뿐이고, 글자 크기 · 선
굵기 · 여백은 스킨 CSS 의 몫이다(§8). 그래서 글자 멤버는 배율만큼 상자가
넓어지고 **그 안에서 줄바꿈이 다시 난다**.

`height:"auto"` 인 멤버는 **가로만 배율을 받는다.** 세로는 계속 내용이
정하므로 그룹의 실제 화면 상자가 `s` 배와 정확히 같지 않을 수 있다 —
**의도한 동작**이고 §40-13 에 남긴다. `"auto"` 를 조용히 숫자로 굳히지
않는다.

2026-09-24 실측: 폭 120 · `height:"auto"` 인 글자와 60×36 도형을 한 그룹
으로 묶고 `se` 를 끌었을 때, 둘의 **가로 배율이 같았고**(1.0952 ·
편차 0) 글자의 `font-size` 는 `12px` 그대로였으며 저장된 `height` 는
`"auto"` 그대로였다.

### 40-5. 회전 — 중심만 옮기고 각도를 더한다

```text
  P = 제스처 시작 시 그룹 바깥 상자의 **중심**(피벗)
  θ = 공통 회전 delta

  c'  = P + R(θ)(c − P)            c 는 그 멤버의 상자 정중앙
  x'  = x + (c' − c)               width · height 는 그대로
  rot'= normalize(rotation + θ)    0 ≤ rot' < 360
```

- 각 멤버의 중심이 **그룹 중심 둘레를 함께 돌고**, 자기 각도에도 **같은
  delta** 가 더해진다. 크기와 내부 상대 배치는 유지된다(실측: 멤버 사이의
  거리가 소수 첫째 자리까지 같았다).
- 회전 중심은 지금까지처럼 요소 상자의 정중앙이다(§4) — 그래서 회전한
  요소의 축평행 상자의 중심이 곧 그 값이고, 프레임이 잰 상자에서 바로
  나온다.
- **30° 자석**(§21-1)을 **공통 delta 에** 건다. 멤버마다 시작 각도가
  다르므로 각자의 절대 각도를 30° 배수에 붙일 수는 없다 — 그룹이 돌린
  양이 30° 근처(±4°)면 정확히 30° 로 돈다. 자석의 단계 · 범위 · "돌지
  않았으면 걸지 않는다"는 단일 회전과 **같은 함수**다
  (`snapCanvasRotation`).
- 각도는 저장 직전에 한 바퀴 안으로 접는다(`normalizeSkinHomeCanvasRotation`).
  실측: 0° · 20° 인 두 멤버를 −45° 돌리면 315° · 335° 가 되고 둘의 차가
  20° 그대로였다.
...
| | 규칙 |
| --- | --- |
| 잠긴 멤버가 **하나라도** 있다 | 크기 조절 · 회전 **둘 다 시작 금지**. 손잡이가 그려지지 않고 draft · Undo 0칸(§39-7 의 그 규칙 · 그 안내 문구) |
| 숨은 멤버 | 그룹 소속 그대로이고 **같은 배율 · 같은 각도**를 받는다 |
| 숨긴 멤버를 위한 DOM | **만들지 않는다.** 저장 geometry 로 계산한다 |
| 숨김 상태 · 그룹 membership | 제스처가 바꾸지 않는다 |
| 보이는 멤버가 둘 미만 | 그룹 선택이 성립하지 않아 손잡이도 없다(§39-11) |

★ **숨은 멤버의 기준점은 되짚어 얻는다.** 프레임은 화면에 없는 요소의
벡터를 보낼 수 없으므로, 부모가 **보고된 멤버 하나**에서 기준점을 저장
좌표계로 되짚는다(§40-8 의 그 식). 그 한 점을 모든 멤버가 공유하므로
숨은 멤버도 같은 변환을 받는다.

★ **`height:"auto"` 인 숨은 멤버가 있으면 회전을 아예 열지 않는다.**
회전은 멤버마다 **중심**이 필요하고, 그 중심에는 세로 길이가 필요하다.
보이는 멤버의 그 길이는 프레임이 재서 보내지만(§40-8 의 `h`), 화면에
없는 `"auto"` 멤버의 길이는 아무도 모른다 — 숫자를 지어내지 않고
회전 손잡이를 그리지 않는다(`studioCanvasV2GroupCanRotate`). 크기
조절에는 이 제한이 없다(높이를 쓰지 않는다).

### 40-7. 공통 배율의 범위 — 멤버마다 자르지 않는다

```text
  scaleMin = max over 멤버 ( 1 / w ,  1 / h )          h 가 숫자일 때만
  scaleMax = min over 멤버 ( 100000 / w , 100000 / h )
```

`1` 은 단일 리사이즈의 바닥과 같은 값이고(`CANVAS_MIN_SIZE`), `100000` 은
좌표 · 크기의 상한(`SKIN_HOME_CANVAS_MAX_COORD`)이다.

★ **멤버마다 따로 자르면 상대 배치와 비율이 그 순간 깨진다.** 그래서
구간을 **먼저 겹쳐** 하나로 만들고(`studioCanvasV2GroupScaleRange`),
화면은 그 범위 안에서만 배율을 만든다. 바닥에 닿아도 모든 멤버가 같은
배율이다(실측: 4000px 안쪽으로 끌었을 때 배율 0.0499 · 편차 0.0001 ·
가장 작은 저장 크기 1.75).

- 부모가 그 범위를 `canvas-group` 으로 내려 주고(§40-8), **확정에서 다시
  본다** — 프레임이 보낸 숫자를 근거로 삼지 않는다.
- 겹칠 구간이 없으면 두 칸 자체를 보내지 않는다. 그때 모서리 손잡이가
  그려지지 않으므로 제스처가 시작되지 않는다.
- `"auto"` 높이는 배율을 받지 않으므로 세로 쪽 제한도 만들지 않는다.
- **페이지 가장자리 clamp(§30-4-1)는 크기 조절에 없다.** 그것은 렌더
  시점의 CSS `clamp()` 이고 단일 리사이즈도 받지 않는다(§40-13).

### 40-8. 메시지 — 새 종류 하나, 늘어난 칸 셋

```text
  canvas-group            parent -> frame     (칸 셋이 늘었다)
    { active, groupId, baseWidth, locked,
      scaleMin, scaleMax, canRotate,
      generation, revision, answering }

  canvas-group-transform  frame  -> parent    (새 종류)
    { groupId, gestureId, phase, kind, scale, angle,
      members: [ { id, vx, vy, h } ],
      generation, revision, requestId }
```

sandbox 봉투의 이름은 `IMORY_CANVAS_GROUP_TRANSFORM` 이고, 기존 origin ·
source · frame · renderSeq 관문을 그대로 지난다. 봉투가 아는 type 은
서른여섯이 됐다.

- `kind` 는 둘뿐이다 — `"resize"` 면 `scale` 이, `"rotate"` 면 `angle` 이
  뜻을 갖는다. **쓰이지 않는 칸도 모양을 지킨다**(`rotate` 의 `scale` 은
  1, `resize` 의 `angle` 은 0) — "안 쓰니 아무 값이나"를 허용하면 뒤에 그
  칸의 뜻이 바뀌는 날 옛 프레임이 조용히 통과한다.
- **기준점(anchor · pivot)은 올라오지 않는다.** 프레임은 저장 좌표계의
  원점을 모른다(프레임 안 요소는 프레임의 원점을 쓴다 — §26-2). 대신
  멤버마다 **차이 벡터**를 보낸다. 거기에는 그 원점이 지워져 있다.

  ```text
    resize   v(m) = (저장 좌표가 가리키는 점) − anchor     도화지 자
    rotate   v(m) = (상자 정중앙)             − pivot      도화지 자
  ```

  한 그룹의 멤버는 전부 같은 좌표 공간이므로(§38-2) 자기 자 위의 값에
  `unitScale`(§39-5)을 곱하면 원점이 같은 한 자 위로 모인다. 그래서
  **보고된 멤버 하나로 기준점을 되짚을 수 있다.**

  ```text
    A = space.x × unitScale − v.x          (y 도 같다)
    A(m) = A / unitScale(m)                멤버마다 자기 자로
  ```

  되짚는 데 쓰는 멤버는 **그룹 명단에서 가장 앞선 보고된 멤버** 하나로
  못박는다 — 어느 멤버로 되짚어도 답은 같지만, 같은 입력이 언제나 같은
  결과를 내야 한다.
- `h` 는 그 멤버가 **지금 화면에서 갖는 세로 길이**(도화지 자)다.
  `height:"auto"` 인 멤버의 중심은 저장값만으로 낼 수 없어서 잰다 —
  §28-3 의 그 원칙 그대로 **잴 수밖에 없는 값 하나만** 보고한다. 저장된
  높이가 숫자면 그 저장값을 쓴다.
- `phase` 는 이동과 같은 셋(`start` · `end` · `cancel`)이고, 끄는 동안의
  중간 보고가 없다. `requestId` 는 `end` 에만 있고, 답은 같은
  `canvas-group` 의 `answering` 으로 돌아온다.
- **전체 Canvas JSON 도 임의의 style 객체도 보내지 않는다.** 멤버 줄은
  네 칸이 전부이고, 모르는 칸이 하나라도 섞이면 메시지 전체가 거부된다.
- `scaleMin`/`scaleMax` 는 **둘이 짝**이고 `min ≤ max` 다. 범위가 없으면
  두 칸 자체를 만들지 않는다. `active:false` 인 해제 메시지에는 세 칸이
  전부 없다.
...
제스처 하나에 **열린 칸 한 개**를 둔다 — 이동과 **같은 칸**이고, 그
칸에 종류(`"move"` · `"resize"` · `"rotate"`)를 함께 적는다. 다른 종류의
`end` 가 그 칸을 쓸 수 없다.

`end` 가 통과하려면 §39-4 의 전부에 더해 이것들이 참이어야 한다.

- 열린 칸의 **종류**가 같다
- 보고된 멤버 명단이 시작 때와 **글자 단위로** 같고, 전부 이 그룹의
  멤버다
- (크기) 배율이 지금 draft 로 다시 구한 **공통 범위 안**이다
- (회전) 지금 draft 로 다시 본 **회전 가능 여부**가 참이다

제스처 중에 아래가 생기면 전부 취소이고 Undo 0칸이다 — 다른 레이어 ·
다른 그룹 선택 · 멤버 변경 · 그룹 해제 · 삭제 · Undo/Redo · working
revision 변경 · Preview 재마운트 · native ↔ sandbox 전환 · HOME 이외
화면 전환 · 늦게 도착한 이전 `requestId` · 시작 때와 다른 명단.

**계획을 전부 만든 뒤에 한 번에 적용한다**(§39-6 과 같은 한 줄 —
`writeStudioCanvasElementChanges(steps)`). 회전은 멤버마다 **두 칸**
(자리 · 각도)을 바꾸므로 계획이 멤버당 두 줄이지만, 서로 다른 칸이라
같은 묶음 안에서 `expected` 를 깨지 않는다.

| 상황 | Undo |
| --- | --- |
| 크기 조절 한 번 성공(이벤트가 몇 번이든) | **1** |
| 회전 한 번 성공 | **1** |
| 시작값으로 돌아와 끝냄(배율 1 · 각도 0) | **0** |
| 취소(Escape · pointercancel · 선택 변경 · 재렌더) | **0** |
| stale · 잠금 거절 · 범위 밖 · 원자적 실패 | **0** |

Undo 한 번이면 모든 멤버가 시작 크기 · 자리 · 각도로, Redo 한 번이면
모두 최종 상태로 돌아간다(working skin 스냅샷 한 칸 — §17-6).

### 40-10. 끄는 동안의 화면

끄는 동안 바뀌는 것은 멤버마다 **화면용 네 칸**이다
(`skin/skin-home-canvas-render.js` §0-4).

| 칸 | 무엇 |
| --- | --- |
| `--imory-canvas-drag-x` · `-y` | 화면 px 의 임시 이동(§39-8 그대로) |
| `--imory-canvas-drag-scale` | **폭 · 높이 값에 곱해지는 배수** |
| `--imory-canvas-drag-rotate` | 저장 각도에 **더해지는** 각도 |

★ **배율이 `transform: scale()` 이 아닌 이유**가 §40-4 의 그 계약이다.
확정이 바꾸는 것은 상자뿐이고 글자 크기가 아니다 — `scale()` 로 보여
주면 글자까지 함께 커졌다가 손을 놓는 순간 원래 크기로 돌아간다.
그래서 배율을 **폭 · 높이 값 자체**에 곱한다. 곱셈은 CSS 가 요소 자기
자(백분율 · cqw · 프레임 내부 자) 위에서 하므로 프레임은 멤버마다 다른
그 자를 알 필요가 없다. `"auto"` 높이에는 곱할 칸이 아예 없어 가로만
배율을 받는다 — **확정 결과와 정확히 같은 그림**이다.

- 저장값 · Canvas JSON · Undo 는 끄는 동안 한 글자도 바뀌지 않는다.
- 외곽선과 손잡이 다섯도 따라가기 루프에서 매 프레임 다시 맞춘다.
- 취소는 그 네 칸을 지우는 것 하나다 — 시작 모양으로 완전히 돌아온다.
- 확정을 보낸 뒤에는 임시 값을 그대로 둔 채 답을 기다린다(상한 4초 —
  §39-8 과 같은 규칙).
- Preview 스크롤 · iframe 경계를 지나도 한 손가락이 같은 제스처에 머문다
  (`setPointerCapture` + window capture).

### 40-11. 모바일

- 모서리 넷과 회전 하나가 **20px** 이다(단일 손잡이 14px 보다 크다).
  보이는 크기가 곧 잡는 크기이고, 그 자리에는 `touch-action: none` 이
  걸려 있다.
- **손가락도 손잡이를 받는다.** 본체 끌기를 손가락에서 막는 이유는
  "세로 스크롤인지 이동인지 가를 수 없다"였는데(§17-2), 손잡이에는 가를
  것이 없다.
- 그 제스처가 이동으로 새지 않는다(실측: 390px 터치 크기 조절에서
  그룹 이동 0회).
- 끄는 동안 페이지가 가로로 밀리지 않는다.
- Layers 의 350ms 길게 누르기(§32-8)는 Preview 의 손잡이에 **적용하지
  않는다** — 손잡이를 짚은 손가락은 그 자리에서 곧바로 시작한다.

### 40-12. 무엇이 그대로인가

크기 조절 · 회전이 바꾸는 것은 멤버의 **자리 · 크기 · 각도 칸**뿐이다
(overlay · `transform` 은 `x`·`y`·`width`·`height`·`rotation`, `pin` 은
`pin.offset` 두 칸 + `width`·`height`·`rotation`). 다음은 글자 단위로
같다.

`canvas.groups`(id · 이름 · 멤버 명단과 그 순서) · `overlays` 와
`props.elements` 의 배열 순서 · `type` · `props` · `slot` ·
`hidden` · `locked` · `follow` · `pin` 의 `target`/`anchor`/`origin` ·
`flow` · 모르는 미래 필드.

Save · 새로고침 · Export/Import · Publish resolve 를 지나도 결과가
유지된다(실측: 이동 → 크기 → 회전 → 이동 뒤의 JSON 이 Export/Import
왕복과 Save/재열기에서 글자 단위로 같았고, `resolve` 가 그 각도와 폭을
그대로 실었다).

### 40-13. 남은 차이

- **`height:"auto"` 멤버가 있는 그룹은 크기 조절 뒤 화면 상자가 배율과
  정확히 같지 않다** — 가로만 배율을 받기 때문이다(§40-4). 거부하지
  않는다.
- **글자 크기는 배율을 받지 않는다**(§40-4). "글자도 함께 커지는 크기
  조절"이 필요해지면 그것은 단일 리사이즈의 계약부터 바꾸는 별도
  작업이다.
- **`height:"auto"` 인 숨은 멤버가 있는 그룹은 회전할 수 없다**(§40-6).
- **페이지 가장자리 clamp 가 크기 조절에는 없다**(§40-7). 그 clamp 은
  렌더 시점의 CSS 이고 단일 리사이즈도 받지 않는다 — 저장값이 화면 밖을
  가리켜도 그려질 때 안으로 당겨진다.
- **스킨이 캔버스 요소에 테두리 · 안쪽 여백을 주면** 손잡이가 놓이는
  합집합(실제 상자)과 부모가 계산하는 저장 상자가 그만큼 어긋난다.
  기준점이 그 차이만큼 밀릴 수 있다 — 지금 계약에서 요소의 상자는
  플랫폼의 몫이므로(§8) 정상 범위에서는 생기지 않는다.
- **변 중앙 손잡이 · 뒤집기 · 중첩 그룹 · 그룹 단위 순서 이동**은 여전히
  없다(§40-1).
- **이 라운드는 migration 도 `APP_BUILD_VERSION` 변경도 하지 않았고
  배포하지 않았다.**
