...
1. 글의 대표 사진
   1. **본문에 넣은 사진 중 대표** — `post_gallery_images`의
      `is_primary`, 없으면 본문 첫 사진(`position` → `id`) →
      `/api/post-cover?image=<사진 id>`
   2. 예전 COVER 업로드 — `post_covers`가 있으면
...
1-1이 1-2보다 먼저인 이유: COVER 업로드 칸은 없어졌고
(`posts/editor/posts-cover-image.js`), 지금 글의 대표 사진은 본문
사진 한 장이다. `post_covers`에는 그 칸이 있던 시절의 행만 남아
있으므로 **새 글에는 아예 행이 생기지 않는다** — 처음 배포에서
카드 배경이 항상 기본 사진/그라데이션이던 원인이 이것이었다
(2026-09-13). 고르는 순서는 갤러리 카드와 같다
...
- 일시적 실패(fetch 실패 · 408 · 425 · 429 · 5xx)는 **최대 3번까지 다시
  시도한다**(1.2초 · 3.5초 간격, 응답에 `Retry-After`가 있으면 그 값이
  이긴다). 설정 없음(`not-configured`)과 4xx는 다시 해도 같으므로 바로
  포기한다.
...
```
카드 설정 version · 기본 카드 사진 주소 · posts.updated_at ·
대표 사진(어느 사진인지까지) · 라벨
```

대표 사진은 "있다/없다"가 아니라 **그 사진의 id**를 섞는다. 본문
글자를 건드리지 않고 대표만 바꾸면 `posts.updated_at`만으로는
구분되지 않기 때문이다.
...
| `image` | **실제 `api/og/post.js`** — 1200×628 PNG · 렌더러에 넘긴 HTML이 설정 화면과 같은 문서(색·글자색·제목 크기·frame) · **기본 사진의 `image_position_x/y`가 그대로 들어가고 설정 화면과 같은 함수에서 나옴** · 글 대표 이미지는 가운데 · **본문 사진 중 대표 지정 한 장이 배경(대표 지정이 없으면 본문 첫 사진), 그것이 예전 `post_covers`와 기본 사진을 이김**, 대표를 바꾸면 `og:image`의 `v`가 달라짐, 비밀글은 본문 사진 id도 나가지 않음 · 자동/사용자 지정 라벨 · **slug·카테고리 메타 줄 없음** · 빈 라벨은 그리지 않음 · **share_label_seq 컬럼이 없어도 카드가 그려짐** · 배경이 프록시 주소(버킷/서명 URL 아님) · 비밀글은 렌더 호출 0 · 설정 없음/렌더 실패 시 기본 카드 · 버전은 실제 변경 때만 달라짐 |
...
1-1. **실패의 정체는 `render-429` — Browser Rendering 한도다**
   (2026-09-13 배포 후 실측). 진단 헤더를 넣고 나서 확정했다:

   | 부르는 방식 | 결과 |
   | --- | --- |
   | 쉬지 않고 6번 연속 | 성공/`fallback:render-429`가 **번갈아** |
   | **15초 간격으로 5번** | **5/5 성공, 전부 첫 시도** |

   즉 단발 크롤은 원래 잘 그려지고, **짧은 시간에 몰릴 때** 한도에
   걸린다. 그래서 재시도 간격을 그 창에 맞췄다(1.2초 · 3.5초 —
   처음 잡았던 250ms · 750ms는 세 번을 1초 안에 다 써 버려 한 번의
   크롤에서 건질 수 있는 것을 놓쳤다).

   저장소 코드가 할 수 있는 것은 여기까지다. 몰리는 양이 한도를
   계속 넘으면 **Cloudflare 쪽 한도를 올리는 것**이 답이다
   (Workers & Pages → Browser Rendering의 사용량/한도).
   지금 상태는 이 한 줄로 언제든 다시 잰다:
