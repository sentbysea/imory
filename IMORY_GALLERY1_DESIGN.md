...
| 설정 | key | 적용 대상 |
| --- | --- | --- |
| 이미지 EXIF 제거 | `strip_image_exif` | 이 블로그에 올리는 이미지 **전부** — 2026-09-12부터 배너·아바타·스킨 이미지 라이브러리까지 포함한다(§13-6) |
...
4. **본문에 붙인 외부 이미지 주소는 대상이 아니다.** 이 저장소에는
   ...
   → **해소됨(2026-09-12) — §13-6.** 본문 사진 업로드도 생겼고
   (`IMORY_POST_BODY_IMAGE_DESIGN.md`), 모든 업로드 경로가 공용
   준비 함수 하나를 지난다.
...
| 경로 | 파일 |
| --- | --- |
| 본문 사진(post · gallery 공통) | `posts/editor/posts-body-images.js` |
| 파비콘 · 커서 | `admin/settings/admin-image-setting.js` |
| 비밀글 지정 이미지 | `admin/settings/admin-settings-category-display.js` |
| 내 배너 | `admin/settings/admin-my-banner.js` |
| 아바타 | `admin/settings/admin-settings-avatar.js` |
| 카테고리 배너 이미지 | `posts/view/posts-view-banner-form.js` |
| 스킨 이미지 라이브러리(Studio) | `studio/images/skin-image-library.js` |

**무엇을 하는가**

1. 디코드 → canvas에 다시 그리기 → 다시 인코딩. 픽셀만 남으므로 EXIF·GPS·XMP뿐
   아니라 **PNG의 tEXt/iTXt 청크**(AI 생성 이미지의 생성 정보가 여기 들어간다)도
   결과 파일에 존재하지 않는다.
2. 긴 변이 **2048px**를 넘으면 비율을 지켜 줄인다. AI 생성 이미지(보통
   1024~1216px)는 해상도가 그대로다.
3. jpeg·webp는 **품질 0.85**로 다시 인코딩한다. png는 **투명한 픽셀이 하나라도
   있으면 png 그대로**(투명을 잃으면 안 된다), 없으면 **jpeg로 바꾼다** — 사진을
   png로 저장한 파일이 용량의 대부분이다. 형식이 바뀌면 파일 이름과 저장 경로의
   확장자도 함께 바뀐다(그래서 경로를 `file.type`으로 만드는 호출부는 **준비한
   뒤에** 경로를 만든다).
4. gif·svg·ico 등은 건드리지 않는다(§13-4와 같은 이유 — 애니메이션이 납작해지고
   벡터가 래스터로 굳는다).
5. 줄이지도 못했고 형식도 그대로인데 결과가 원본보다 크면 **원본을 그대로 둔다**
   (설정이 켜져 있을 때는 예외 — 아래).

**설정이 실제로 바꾸는 것**

압축은 설정과 **무관하게 항상** 돈다. 그래서 설정이 꺼져 있어도 다시 인코딩된
파일에는 메타데이터가 남지 않는다. 설정이 정하는 것은 **실패했을 때**다.

| `strip_image_exif` | 준비에 실패하면 |
| --- | --- |
| 켜짐 | 올리지 않는다(§13-4의 기존 계약 그대로 — 저장을 멈추고 안내한다) |
| 꺼짐 | 원본을 그대로 올린다(압축은 최적화일 뿐이다) |

Studio의 스킨 이미지 라이브러리만 `stripMetadata: false`다 — 그 화면은 블로그의
보호 설정을 읽지 않는다. 성공하면 어차피 다시 인코딩된 파일이다.

**실측(`skin/skin-gallery-e2e-test.mjs --only=protect`)**

2600×1400 PNG를 본문 사진으로 올렸을 때:

| 원본 | 올라간 것 |
| --- | --- |
| 투명 없음 · 1,334,858 bytes | **JPEG** 2048×1103 · 986,686 bytes · 경로 `.jpg` |
| 투명 있음 · 1,334,867 bytes | **PNG** 2048×1103 · 1,114,291 bytes · 경로 `.png` |
---
