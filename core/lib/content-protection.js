...
/* =========================================================
   ...
   ★ 방법
   canvas에 그린 뒤 다시 인코딩한다. 그러면 픽셀만 남고 EXIF/GPS/
   XMP 같은 메타데이터 블록은 **결과 파일에 존재하지 않는다**. 별도
   파서를 넣어 특정 태그만 지우는 것보다 확실하다(모르는 태그가
   남는 일이 없다).

   ★ 방향(Orientation) 보정
   EXIF Orientation을 지우면서 그림만 그대로 두면 세로로 찍은 사진이
   눕는다. createImageBitmap의 imageOrientation:"from-image"로 방향을
   **픽셀에 반영해서** 그린다 — 그래서 결과 파일은 방향 태그가 없어도
   똑바로 보인다.

   ★ GIF는 건드리지 않는다
   GIF에는 EXIF 세그먼트가 없고, 다시 인코딩하면 애니메이션이 첫
   프레임으로 납작해진다. 잃는 것이 얻는 것보다 크다.

   ★ 실패하면 올리지 않는다
   사용자가 "메타데이터를 지워라"라고 켜 둔 설정이다. 조용히 원본을
   올리는 것은 그 설정을 배신하는 일이라, 실패는 실패로 돌려준다.

   -> { file, changed, error }
========================================================== */

const IMORY_EXIF_REENCODE_MIME =
  [
    "image/png",
    "image/jpeg",
    "image/webp"
...
const IMORY_EXIF_REENCODE_QUALITY =
  0.92;


async function stripImageExifIfNeeded(
  file,
  shouldStrip
) {
  ...
  if (
    !IMORY_EXIF_REENCODE_MIME.includes(file.type)
  ) {

    /* GIF 등 — 위 주석의 이유로 그대로 둔다 */
