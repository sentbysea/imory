...
**사실(부분 구현)** — 공개 시작 페이지(`index.html`, 4-9의 "공개 시작 페이지")에 한해 다음이 실제 코드로 존재한다: `<html data-theme>`(`index.html` `<head>` 인라인 스크립트 `initSystemTheme`), `prefers-color-scheme` 폴백, `localStorage`(`imory_system_theme`) 저장/우선, FOUC 방지(CSS 로드 전에 `<head>` 최상단에서 `data-theme` 조기 적용), 클릭 토글(`core/theme-toggle.js`), 다크 오버라이드 토큰(`core/design-tokens.css` `:root[data-theme="dark"]`). 관리자/에디터 등 나머지 시스템 UI 화면에는 아직 이 토글이 연결되어 있지 않다 — 그 화면들은 여전히 "계획"이다. 확정된 정책(공개 시작 페이지는 아래를 이미 만족, 나머지 화면 구현 시에도 동일 정책 적용):
...
**사실 + 지침** — 공개 시작 페이지 다크 모드 적용 중 iOS Safari에서 화면 맨 위/아래가 다크 모드에서도 흰색으로 보이는 버그가 있었다(`home/home-base.css`의 `html, body`가 다크 모드와 무관하게 `background: #ffffff` 리터럴로 고정돼 있던 게 원인). 원인은 두 갈래이고, **다크 모드를 새로 붙이는 모든 화면**(관리자/에디터 등)이 앞으로 똑같이 겪을 수 있으므로 반드시 아래 두 가지를 함께 적용한다:
