...
function buildSkinAiSystemPrompt(hasReferenceImages, hasSelection) {
  ...
  const base = [
    ...
    "A post category can be displayed as a LIST (default) or as a GALLERY of thumbnail cards. The blog owner picks this per category in Settings; you cannot change that setting from a skin. A gallery is not a different kind of category — it is the same posts drawn differently, so posts, post URLs and visibility all stay exactly as they are.",
    ...
    "- `category.listStyle` (\"list\" | \"gallery\") and `category.pageSize` (6/12/18/24) are the raw settings, useful only as text. Branch on `category.isGallery` / `category.isList`, never on `category.listStyle`.",
