...
function buildSkinAiSystemPrompt(hasReferenceImages, hasSelection) {
  ...
  const base = [
    ...
    "",
    "## Owner settings the skin reads (colours · HOME photo layout · D-day)",
    "The owner picks these in Studio; they live in SkinPackage.regions (entries named theme_colors, home_photos, dday — you never write or change regions). The skin only READS them:",
    "- Colours: four roles arrive as CSS custom properties on the skin root — `--imory-color-background`, `--imory-color-text`, `--imory-color-accent` (point 1), `--imory-color-accent-2` (point 2). Read them WITH a hex default: `var(--imory-color-text, #1b2340)`. Derive lighter lines, muted text and decoration from these four with color-mix(); do not invent more colour options. \"배경을 어둡게\" on a skin that reads these variables is the owner's colour setting — change the var() defaults only if the owner asks to change the skin's own default look. (`--imory-color` without a suffix is still the highlight card colour from data-imory-color.)",
    "- HOME photos: a block `data-imory-photos=\"set\"` holds items `data-imory-photos-item`, each normally one image slot guarded by `data-imory-if=\"images.<slot>\"`. The platform counts the filled ones and stamps `data-imory-photos-layout=\"empty|hero|pair|triptych\"` on the set (0/1/2/3+ photos, or the owner's choice in home_photos, never more than the filled count; at most 3 are shown) and `data-imory-photos-state=\"shown|rest|empty\"` + `data-imory-photos-position=\"1|2|3\"` on the items. Style each layout from those attributes; never count photos yourself with :nth-child and never write these runtime attributes into HTML. Keep every photo an image slot — never a CSS background-image.",
    "- D-day: `settings.dday` is null unless the owner set a date. Guard with `data-imory-if=\"settings.dday\"` and bind `settings.dday.display` (\"730\" counting the day itself as 1, or \"D-12\" before the date) and `settings.dday.label`. Never type a number yourself; there is no visitor-count binding, so do not draw a visitor counter.",
    "",
