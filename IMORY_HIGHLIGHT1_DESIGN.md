...
## 12. 읽는 모습 다듬기 (2026-09-13, 실기기 확인 뒤)

아이폰 실사용에서 나온 다섯 가지. 저장 구조(§1)와 위치 식별 규칙(§3)의 **판정**은
그대로고, 평문에 줄바꿈이 들어온 것 하나만 §3의 보강이다.

### 12-1. 발췌문에 줄바꿈이 남는다

**문제**: 두 문단에 걸쳐 고른 발췌문이 메모 팝업과 카드에서 한 덩어리로 붙어
나왔다(`"…알았는데."무뚝뚝하게 중얼거리면서…"`). 평문 색인이 텍스트 노드를
아무 구분 없이 이어 붙였기 때문이다.

**바꾼 것**(`posts-view-highlight-anchor.js`): 화면에서 줄이 바뀌는 자리에
평문에도 `"\n"`을 넣는다.

| 무엇 | 몇 줄 | 왜 |
| --- | --- | --- |
| `<br>` | 1 | 이 에디터는 Enter를 눌러도 `<p>`를 만들지 않는다 |
| `.post-body-paragraph-gap` | 2 | `<br><br>`(문단 경계)이 그릴 때 이 span 하나로 바뀐다 |
| 블록 요소(`P`/`DIV`/`LI`/`H1`…) | 1 | 옛 글의 `<p>` 문단, 복사 상자·구분선 |

`<br>`과 간격 span은 **쌓이고**(빈 줄 세 개는 세 줄), 요소 경계는 쌓이지
않는다(`<div><p>`가 두 줄이 되지 않는다). 글자가 더 나오지 않으면 버리므로
본문 끝의 `<br>`이 발췌문 뒤에 빈 줄을 남기지 않는다.

**옛 발췌문**: 이 규칙 이전에 저장된 것은 문단 사이가 붙어 있어 그대로는
찾히지 않는다. 그래서 정확히 일치하는 후보가 하나도 없고 저장된 발췌문에
`"\n"`이 없을 때만, 본문 쪽의 `"\n"`을 건너뛰며 맞춰 보는 두 번째 탐색을
한다(`postHighlightLenientMatches`). 그 경로에서는 문맥 비교도 본문에서 `"\n"`을
걷어낸 뒤 하고, `text_start` 비교도 그 앞에 끼워 넣은 `"\n"` 수를 빼고 한다
(색인이 그 자리를 `breaks`에 적어 둔다). **"확실할 때만 연결한다"는 판정
자체는 그대로다** — 후보가 여럿이면 여전히 연결하지 않는다.

발췌문의 끝이 `"\n"`일 수는 없다 — 양 끝 공백을 잘라내는 기존 규칙이 그대로
적용된다. 그래서 범위 계산이 줄바꿈 위에 놓이는 일이 없다.

### 12-2. 메모 말풍선은 하이라이트 **덩어리** 위에 선다

읽기 상태의 말풍선만 기준이 바뀌었다: 누른 줄이 아니라 같은 id로 칠해진 조각
전부를 감싼 사각형이다(`resolvePostHighlightBlockAnchor`). 누른 줄 위에 띄우면
여러 줄짜리 하이라이트의 나머지가 말풍선에 가려, 무엇에 단 메모인지 보이지
않은 채 메모만 읽게 된다.

주인장의 메모/삭제 메뉴는 §5-3 그대로 **누른 줄**을 따라간다 — 거기서는 어느
줄을 눌렀는지가 곧 조작 대상이다.

말풍선에는 꼬랑지가 생겼다. 판은 화면 안으로 당겨지므로 꼬랑지의 가로 자리는
팝오버가 재어 `--imory-popover-arrow-x`로 넘기고(`posts-view-popover.js`), 판이
끝까지 밀려도 꼬랑지는 기준 사각형의 가운데를 계속 가리킨다. 판 자체는
`rgba(255,255,255,0.88)` + `backdrop-filter: blur(3px)`이라 아래 원문이 한 겹
비치되 메모 글씨는 또렷하다. 반투명 판에 테두리를 두면 꼬랑지 밑동에 실선이
가로질러 남으므로 테두리 대신 그림자를 쓴다.

### 12-3. 하이라이트에서 점선을 걷어냈다

메모가 달린 하이라이트의 점선 밑줄과 하이라이팅 모드의 점선 테두리를 없앴다.
본문 글줄 사이에 실선도 점선도 아닌 줄이 하나 더 생겨 읽는 흐름을 끊었다 —
칠해진 색이 이미 "여기 있다"와 "누를 수 있다"를 모두 말한다. 모서리도
`border-radius: 0`으로 각졌다. 남는 표시는 커서와, 메모 카드에서 원문으로
왔을 때 잠깐 붙는 `[data-post-highlight-focus]` 외곽선뿐이다.

### 12-4. 판과 버튼이 각졌다

메모 팝업·팝오버·textarea·취소/SAVE가 알약과 큰 둥근 모서리를 버리고
`core/design-tokens.css`의 radius 토큰(2/4/6px)을 쓴다. 버튼 두 개는
`core/components/button.css`의 `.imory-button` 패턴(각진 사각형 · 얇은 테두리 ·
흰 배경 · 토큰 색)을 그대로 따른다.

**그 파일을 읽지 않고 `posts-highlight.css`에 같은 모양을 적는 이유**:
이 스타일시트는 design tokens를 의도적으로 로드하지 않는
`studio/preview/preview-frame.html`에도 실린다(그 문서는 "아무 전제 스타일 없이
스킨 CSS만으로" 그려야 한다). 그래서 `var()`마다 같은 값의 fallback을 같이
적어 두 문서가 같은 모양으로 나오게 한다.

다른 점 하나 — 기본 `.imory-button--primary`는 평소 회색이고 hover에서만 핑크가
드러나지만, 이 둘은 손가락으로 쓰는 팝업 안에 나란히 서 있어 hover가 없다.
그래서 SAVE만 그 hover 상태를 평상시 모습으로 올려 취소와 구별한다(토큰 밖의
새 색은 쓰지 않는다).

### 12-5. 토스트가 아이모리 핑크다

`rgba(51,51,51,0.92)` 회색 판 → `rgba(217,128,162,0.85)`. 잠깐 떴다 사라지는
알림이라 한 겹 더 비치는 편이 덜 가린다. 실패 토스트만 진한 톤을 유지한다 —
실패는 알림이 아니라 결과라 읽히지 않으면 안 된다.

값은 `--system-toast-bg` / `--system-toast-error-bg`로 받는다. 나중에 Settings에서
admin background / point color를 고르게 되면 이 판도 그 범위에 들어가는데,
그때 `:root`에 변수 하나만 얹으면 되고 이 파일은 고치지 않는다.

### 12-6. 검증

`posts/posts-highlight-e2e-test.mjs --only=break` 23건이 위 다섯을 본다 —
저장된 발췌문의 `"\n\n"`, 팝업/카드에 그려진 줄바꿈, 옛 발췌문의 재연결,
말풍선이 덩어리 **전체** 위에 서는 것(마지막 줄을 눌러도), 꼬랑지와 반투명,
점선 없음, 각 모서리의 실제 `border-radius`, 토스트 색과 알파. 전체 스위트는
110건 통과(mock 테스트 — 실제 DB·배포·실기기 확인은 별개다).

---

## 13. 남은 차이 (아직 구현되지 않은 것)
