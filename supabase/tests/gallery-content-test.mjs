...
await db.exec(`
  ...
  -- The body is written by the editor now, not generated by the photo save
  -- function, so the fixture supplies one (issue_gallery_read_token reads it).
  insert into post_contents(post_id,content) values (10,'<p>fixture body</p>');
...
await db.exec(migration);
// Photos moved into the shared post body: the save function must stop writing
// post_contents. Applied twice for the same idempotency reason as above.
const bodyImages = readFileSync(new URL('../migrations/20260912100000_post_body_images.sql', import.meta.url),'utf8');
await db.exec(bodyImages);
await db.exec(bodyImages);
...
// The body belongs to the editor now. Saving photos must leave whatever the
// user wrote alone -- the old function generated the whole body from the photo
// list and overwrote it on every save.
await db.exec(`reset role; insert into post_contents(post_id,content) values (10,'<p>내가 쓴 글</p>')
  on conflict (post_id) do update set content = excluded.content; set role authenticated;`);
const bodyNow = async () => {
  await db.exec('reset role;');
  const row = (await db.query('select content from post_contents where post_id=10')).rows[0];
  await db.exec('set role authenticated;');
  return row.content;
};
await db.query('select * from save_own_gallery_images($1,$2::jsonb)',[10,JSON.stringify([a,b])]);
assert.equal(await bodyNow(),'<p>내가 쓴 글</p>');
console.log('PASS saving photos no longer generates or overwrites the post body');
// A post can lose its last photo. The empty set is valid and reports the files
// nothing references any more.
const orphans = (await db.query('select * from save_own_gallery_images($1,$2::jsonb)',[10,'[]'])).rows
  .map(row => Object.values(row)[0]).sort();
assert.deepEqual(orphans,[a.storage_path,b.storage_path].sort());
assert.equal((await db.query('select * from get_own_gallery_images(10)')).rows.length,0);
assert.equal(await bodyNow(),'<p>내가 쓴 글</p>');
console.log('PASS removing every photo is allowed, reports orphan files and keeps the body');
