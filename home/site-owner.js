/* =========================================================
   ...
   세 가지 상태를 명확히 구분한다:
   - 유효한 slug segment 자체가 없음(루트 "/" 등) →
     { scoped: false, ownerId: null } (무필터, 레거시/단일
     사용자 배포 호환)
   - slug segment가 있고 profiles에서 정상 조회됨 →
     { scoped: true, ownerId: "<uuid>" }
   - slug segment가 있는데 조회 실패(존재하지 않는 slug 등) →
