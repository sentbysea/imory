...
| 형광펜 높이 · 문단 강조선 · 캔버스 배경 사진 · 에디터 컬러피커 | [IMORY_EDITOR_DECOR_DESIGN.md](./IMORY_EDITOR_DECOR_DESIGN.md) |
...
| `posts/posts-editor-decor-e2e-test.mjs` | 8951 | 본문 장식 — 컬러피커(끄는 내내 열림·선택 유지·실시간 미리보기·Apply/Cancel/Escape/바깥클릭·드래그 전체가 undo 한 칸·redo·**선택만 있고 가만히 둘 때 selectionchange 무한 고리가 돌지 않음**) · `[picker/native]` **기본(OS) 색상 선택기**(판정 규칙 native/custom·씨앗 적용·조정 중 본문 노드를 다시 만들지 않음(MutationObserver 0)·문서 선택을 다시 설정하지 않음(selectionchange 0)·undo 한 칸·강조선도 선택이 사라진 뒤 이어짐. Playwright WebKit에는 `<input type="color">`도 `maxTouchPoints`도 없어 **아이폰 자동 판정과 OS 창 거동은 실기기 항목**) · `--only=picker`의 **`[picker/touch]` 절은 반드시 `--browser=webkit`으로도 돌린다**(WebKit은 터치에서 pointerdown preventDefault 뒤 click을 만들지 않아, 마우스/chromium만으로는 아이폰에서 버튼이 안 눌리는 것을 못 잡는다) · 형광펜 재적용이 중첩되지 않음 · 형광펜 높이 · 문단 강조선(문단 전체·여러 줄 한 줄기·대사 자동과 개별 해제·수동 우선·개별 색·공개 뷰어) · `--only=gap`: 강조선 거리 세 벌(옛 프리셋 12px 유지·0 보존·범위 자르기)과 `[numbers]` 네모 숫자 칸(직접 입력·저장 왕복·범위) · `--only=fixed`: 확대 50~150%·**사진을 캔버스보다 작게 줄이기**(드러난 자리는 배경색, 덮개는 사진 위에만)·흐림이 크기를 바꾸지 않음·**이미지 크기 고정**(같은 너비의 서로 다른 높이에서 사물 크기 동일, 첫 페이지 글 길이 무관, 켤 때 현재 크기 유지, 옛 확대 값이 열기만으로 깎이지 않음) · `--only=toolbar`: **정확히 세 줄**(page break·H·P·L·clear / FORMAT·B·I·U·S·photo·preset / 오른쪽 undo·redo)·축약 라벨의 접근성 이름·`[strike]` 취소선(혼합·undo/redo·저장 왕복·옛 strike/del·뷰어·발췌) · `--only=excerpt`: 발췌 설정(강조선 색 견본 제거·본문 툴바 H/P/L는 유지·size 한 줄·크기 표시가 대지 우측 위 바깥·페이지마다 갱신·**사진 교체 후에도 프리셋 덮개/흐림 유지**·reset) · **저장되는 PNG의 실제 픽셀**(배경·강조선·형광펜 높이·**흐림은 `ctx.filter`가 없는 환경에서도 구워진다** — WebKit SKIP 없음, 미리보기 스크린샷과 번짐 폭 비교 + 픽셀 폴백이 σ만큼 흐리는지 직접 호출·**줄인 배경의 자리·크기가 화면과 일치**) · 모바일 터치와 기존 제스처 · 프리셋 왕복과 옛 프리셋 기본값 |
