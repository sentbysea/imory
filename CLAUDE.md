...
| **Sandbox 스킨**(별도 origin iframe) · `renderMode` · 기능 플래그 · frame CSP/nonce · 호스트 분기 · 프레임에 넘기는 데이터 투영 · **프레임 안 이동(navId)** | [IMORY_SANDBOX_SKIN_DESIGN.md](./IMORY_SANDBOX_SKIN_DESIGN.md) (**§G · §H · §J 만 현재 구현** — §A~§F 는 설계이고 여러 곳이 어긋나 있다, 문서 머리말 표 참고) |
