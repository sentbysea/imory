...
| **Sandbox 스킨**(별도 origin iframe) · 기능 플래그 · frame CSP · 호스트 분기 | [IMORY_SANDBOX_SKIN_DESIGN.md](./IMORY_SANDBOX_SKIN_DESIGN.md) (§G만 현재 구현) |
...
| `skin/sandbox/skin-sandbox-e2e-test.mjs` | 8957 + 8958 | SANDBOX-0 빈 프레임 · origin 격리 — **두 개의 실제 origin**(포트로 가름)을 띄우고 둘 다 배포되는 그 `functions/_middleware.js`에 통과시킨다. 메인 origin의 `/skin/sandbox/frame.html`이 404 · sandbox origin에서 앱/supabase client가 안 나옴 · 예상 못 한 Host 거부 · 실제 CSP 헤더 전문과 요청마다 바뀌는 nonce · 기능 플래그 OFF면 iframe이 0개 · `IMORY_FRAME_READY`→`IMORY_FRAME_ACK` 왕복 · 위조 origin/다른 source/알 수 없는 type/모르는 payload 키 거부 · 프레임에서 parent DOM·parent localStorage·fetch가 막힘 · 390px 넘침 0 · 메인 origin 회귀. 단위 테스트는 `node skin/sandbox/skin-sandbox-unit-test.mjs`(플래그·메시지 검증·호스트 분기·CSP, 브라우저 없이) |
