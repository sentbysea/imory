...
| **자르기 우선순위**(스킨 CSS 의 `!important` 보다 자르기가 이긴다 — 렌더러가 저장된 자르기 규칙에서 `@layer imory-crop-guard` 안의 `!important` 보호 규칙을 만든다 · 임시 미리보기는 같은 선언을 inline `!important` · 저장/Export 에는 보호 규칙이 없다) · **프레임이 스킨의 자리를 채우는 방식**(`--imory-crop: fill-width / fill / fill-absolute` — px 폭을 굽지 않는다) | [IMORY_IMAGE_CROP_PRIORITY_DESIGN.md](./IMORY_IMAGE_CROP_PRIORITY_DESIGN.md) · [skin/skin-render.js](./skin/skin-render.js) `buildSkinCropGuardCss` · [studio/preview/preview-bridge.js](./studio/preview/preview-bridge.js) `inspectorCropFillOf` |
