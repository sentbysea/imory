...
| `posts/posts-editor-decor-e2e-test.mjs` | 8951 | 본문 장식 — 컬러피커(끄는 내내 열림·선택 유지·실시간 미리보기·Apply/Cancel/Escape/바깥클릭·드래그 전체가 undo 한 칸·redo) · 형광펜 재적용이 중첩되지 않음(부분/여러 구간/다른 서식 보존/옛 중첩 데이터) · 형광펜 높이(줄바꿈·페이지 높이 불변, 100%는 예전 방식, 직각) · 문단 강조선(문단 전체·여러 줄 한 줄기·대사 자동과 개별 해제 구분·수동 우선·개별 색·출처·공개 뷰어) · 캔버스 배경(가로/세로 사진 빈틈없이 덮기·비율 유지·중심 자르기·흐림 여유·덮개/쌓임 순서·캔버스 비율이 달라도 중심 유지·여러 장 공통+개별 보정) · Editor 조작(진입점 하나·조정 모드에서만 드래그·Escape/reset) · **저장되는 PNG의 실제 픽셀**(배경·강조선·형광펜 높이·흐림이 진짜로 구워지는지) · 모바일 터치와 기존 제스처 · 프리셋 왕복과 옛 프리셋 기본값 |
