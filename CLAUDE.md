...
| `admin/share-card-e2e-test.mjs` | 8954 | 글 공유 카드 — SETTINGS > SHARE 탭(기존 BANNER 기능 그대로 + CARD 설정 저장/재로드/version) · 카드 레이아웃 실측(1200×628 · **바닥 100px X UI 데드존** · 좌우 48px · 긴 제목 2줄 · 배경이 바뀌어도 제목 자리 고정 · WHITE는 짙은 회색) · 미리보기 비율과 즉시 반영 · **실제 Pages Function**(`functions/_middleware.js` · `functions/api/og/post.js`)의 og/twitter meta와 1200×628 PNG · 비밀글/비공개 글의 제목·발췌·대표 이미지 비노출 · 렌더러 미설정/실패 시 기본 카드 · **같은 post+v 재요청에 렌더 호출 0**(Workers Cache)이면서 **캐시가 공개 여부 확인을 건너뛰지 않는가**(비공개 전환 즉시 차단·항목 삭제, 공개 전환 즉시 복구) |
