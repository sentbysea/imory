...
| 템플릿이 받는 데이터(Skin Data Contract) | [docs/contracts/AI_SKIN_PHASE1C_PAGE_CONTRACT.md](./docs/contracts/AI_SKIN_PHASE1C_PAGE_CONTRACT.md) |
| 그 밖의 모든 문서 · 옛 이름 별칭 표 | [docs/INDEX.md](./docs/INDEX.md) |

### 읽기 범위와 Context

- 먼저 `rg`로 관련 위치를 좁히고 필요한 범위만 읽는다. 큰 파일 전체를 반복해서
  읽지 않는다.
- 같은 세션에서 이미 읽은 문서나 파일은 변경되지 않았다면 다시 전부 읽지 않는다.
- 범위 밖 문제를 발견해도 관련 파일과 문서를 자동으로 계속 확장해 읽지 않는다.
  현재 작업을 막는 문제만 확인하고 나머지는 최종 보고에 남긴다.
...
- 구현 중에는 새 집중 테스트와 직접 관련된 Chromium 테스트를 우선 실행한다.
- WebKit·넓은 회귀 테스트·배포 검증은 기준 문서가 요구하거나 마일스톤을 완료할
  때 실행한다.
- 보안, 인증, 권한, DB migration, 저장 데이터 호환성 변경은 이 축소 규칙의
  예외이며 필요한 검증을 생략하지 않는다.
- 통과한 테스트는 로그 전문을 최종 보고에 붙이지 않고 테스트 이름과 통과 수만
  적는다.
- 실패한 테스트만 핵심 로그, 원인, 변경 전 재현 여부를 자세히 남긴다.
- 범위 밖 문제를 발견했다는 이유만으로 테스트 범위를 자동 확장하지 않는다.
...
- 문서는 성격대로 나뉜다. 어느 폴더에 무엇이 있는지는 아래가 전부다.

| 폴더 | 무엇이 있나 |
| --- | --- |
| `docs/contracts/` | 지금 코드가 강제하는 계약 |
| `docs/architecture/` | 전체 구조와 경계 |
| `docs/features/` | 구현된 기능의 상세 설계 |
| `docs/features/*/history/` | 현행 기능을 이해할 때 필요한 라운드 기록 |
| `docs/plans/` | 아직 구현을 뜻하지 않는 계획 |
| `docs/archive/` | 철회되거나 대체된 기록 — **현행 계약의 근거로 쓰지 않는다** |
| `core/design-tokens-mapping.md` | 코드와 가까운 참조 문서라 `core/`에 둔다 |
...
- 새 문서를 만들거나 어떤 문서가 대체되면 [docs/INDEX.md](./docs/INDEX.md) 의
  ...
  문서는 없는 문서나 마찬가지다.
- 문서를 옮길 때는 `git mv`를 쓴다. 파일명을 바꾸기 전에는 코드 주석과 문자열
  참조를 전수 조사한다. 이동과 동시에 [docs/INDEX.md](./docs/INDEX.md) 와 모든
  상대 링크를 갱신하고, Markdown 링크 전수 검사에서 **새 실패가 0** 이어야 한다.

### 작업 시작과 마감

- 시작 전에 `git status`를 확인한다.
- 기존 변경은 사용자나 다른 작업의 것일 수 있으므로 임의로 원복하거나 덮어쓰지
  않는다.
- 한 작업의 변경만 경로를 지정해 stage하며 `git add -A`를 쓰지 않는다.
- 요청받은 범위 밖 문제는 임의로 함께 고치지 않고 최종 보고에 남긴다.
- commit, push, 배포 여부를 각각 구분해 보고한다.
