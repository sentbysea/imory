...
| **Skin Studio 화면 구조**(상단 세 그룹 · Publish 만 primary · Import|Export 한 덩어리) · **왼쪽 패널**(Select · Images · Dock 이 같은 자리, Images/Dock 을 다녀와도 선택 유지, Dock 사본 보존) · Preview 위에는 테두리·이름표·핸들만 · **Undo/Redo**(working draft 변경 기록) · 좁은 화면(두 줄 + ··· 메뉴 · 아래 시트) | [IMORY_STUDIO_SHELL_DESIGN.md](./IMORY_STUDIO_SHELL_DESIGN.md) · [studio/studio-shell.js](./studio/studio-shell.js) · [studio/studio-history.js](./studio/studio-history.js) · [studio/studio-shell.css](./studio/studio-shell.css) |
